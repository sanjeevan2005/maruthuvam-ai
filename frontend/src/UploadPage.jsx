  ...
  try {
    ...
    if (selectedImageType === 'xray') {
      try {
        ...
        // Fallback to prediction response if report generation fails
        reportData = {
          report: predictionRes?.data?.gemini_analysis || 'AI analysis completed.',
          disease: predictionRes?.data?.predictions?.[0]?.[0] || 'Analysis',
          symptoms: predictionRes?.data?.predictions?.map(p => p?.[0]).slice(0,3) || []
        };
      }
    } else {
    ...
    console.error(err);
    const message = err?.response?.data?.detail || err?.message || 'An error occurred during upload or analysis. Please try again.';
    setError(typeof message === 'string' ? message : 'An error occurred during upload or analysis. Please try again.');
  } finally {
    setUploading(false);
  }
};
